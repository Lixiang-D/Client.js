...
util.applyBindings = function (bindings, pattern) {
  ...
  if (bindings[pattern.graph] || pattern.graph)
    newBindings.graph = bindings[pattern.graph] || pattern.graph;
  return newBindings;
};
...
namespace('sd', 'http://www.w3.org/TR/sparql11-service-description/#', [
  'graph',
]);
