...
var XSD = 'http://www.w3.org/2001/XMLSchema#',
  ...
  XSD_DATETIMEDURATION = XSD + 'dayTimeDuration',
    ...
    return (function (operator, argumentExpressions) {
      return function (bindings) {
        ...
        for (var i = 0; i < argumentExpressions.length; i++) {
          ...
          case 'numeric':
            if (!isNumeric(args[i])) throw Error('Argument for ' + args[i] + ' must be numeric');
          ...
          case 'boolean':
            ...
            break;
          case 'dateTime':
            if (!isDatetime(args[i])) throw Error('Argument for ' + args[i] + ' must be of type xsd:datetime');
            args[i] = new Date(N3Util.getLiteralValue(arg));
            break;
          case 'string-literal':
            if (!isStringLiteral(args[i])) throw Error('Argument ' + args[i] + ' must be a string literal');
            break;
          case 'simple-literal-or-string':
            if (!isSimpleLiteral(arg) && !isString (arg[i])) throw Error('Argument ' + args[i] + ' must be of type xsd:string or a simpleliteral');
        ...
        case 'numeric':
          ...
          var type = isNumeric(origArgs[0]) ? N3Util.getLiteralType(origArgs[0]) : XSD_NUMERIC.INTEGER;
          return '"' + result + '"^^' + type;
        case 'boolean':
...
function isNumeric(a) {
  return numeric_types.indexOf(N3Util.getLiteralType(a)) > -1 ? true : false;
}

function isInteger(a) {
  return N3Util.getLiteralType(a) === XSD_NUMERIC.INTEGER;
}
...
var operators = {
  ...
  strlen: function (str) {
    return this.strdt(N3Util.getLiteralValue(str)
      .length, XSD_INT);
  },
  ...
  concat: function () {
    ...
    if (lang) return this.stlang(lexicalForm, lang);
    if (string) return this.stdt(lexicalForm, XSD_STRING);
  ...
  replace: function (subject, pattern, replacement, flags) {
    ...
    return subject.replace(N3Util.getLiteralValue(pattern), N3Util.getLiteralValue(replacement), !flags || N3Util.getLiteralValue(flags));
  },
  ...
  rand: function () {
    return this.stdt(Math.random(), XSD_NUMERIC.DOUBLE);
  },
  // 17.4.5 Functions on Dates and Times
  now: function () {
    return this.stdt(now.toISOString(), XSD_DATETIME);
  },
  year: function (dt) {
    return dt.getYear();
  },
  ...
  seconds: function (dt) {
    return this.stdt(dt.getSeconds(), XSD_NUMERIC.DECIMAL);
  },
  timezone: function (dt) {
    //if (this.tz(dt) === '""') throw new Error('TIMEZONE requires timezone added to dateTime');

    //var offset = dt.getTimezoneOffset();
    //return this.stdt(dt.getTimezone(), XSD_DAYTIMEDURATION);
